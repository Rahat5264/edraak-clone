...
export async function generateMetadata() {
  if (!prod) return { title: "Cutting Optimization Module | Edraak Systems", alternates: { canonical: SITE_URL + "/products/cutting-optimization-module" } }
  const title = prod.title + " | Edraak Systems"
  const description = prod.desc || prod.summary || "Cutting optimization module by Edraak Systems."
  const url = SITE_URL + "/products/cutting-optimization-module"
  return {
    title,
    description,
...
export default function CuttingPage() {
  ...
  const jsonLd = {
    ...
    url: SITE_URL + "/products/cutting-optimization-module",
  ...
  return (
    <>
      ...
      {prod && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />}
    </>
  )
}
